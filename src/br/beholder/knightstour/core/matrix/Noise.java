 ...
 */
package br.beholder.knightstour.core.matrix;
...
public class Noise {
    ...
    public boolean[][] getAsBool() {
        boolean [][] matrix = new boolean[grid_.length][grid_[0].length];
        for(int i = 0;i < grid_.length;i++) {
            for(int j = 0;j < grid_[0].length;j++) {
                if(Math.abs(grid_[i][j])<1){
                    matrix[i][j]=false;
    ...
    public static void main(String[] args) {
        Noise n = new Noise(null, 1.0f, 15, 15);
        n.initialise();
        n.printAsCSV();
        System.out.println("\n\n\n\n");
        n.printAsbool();
